...
@SpringBootTest
class PermissionControllerTest extends BaseTestContainerTest {
  ...
  @Test
  @DisplayName("権限を持つロールで、権限マスタを検索できること")
  @WithMockUser(authorities = "permission:read")
  void test1() throws Exception {
    mockMvc
        .perform(get("/api/system/permissions"))
  ...
  @Test
  @DisplayName("権限を持たないロールでは、権限マスタを検索できないこと")
  @WithAnonymousUser
  ...
  @Test
  @DisplayName("権限を持つロールで、権限マスタCSVを出力できること")
  @WithMockUser(authorities = "permission:read")
  void test3() throws Exception {
    mockMvc
        .perform(get("/api/system/permissions/export/test.csv"))
        .andExpect(status().isOk())
